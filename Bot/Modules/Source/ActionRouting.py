...
from Bot.Modules.Shared.Query import InsertUser, InsertGroup, CheckUserExists, CheckGroupExists
    ...
    match query.data:
        case "add_members":
            if not CheckUserExists(query.from_user.id):
                InsertUser(query.from_user.id, query.from_user.username, query.message.chat.title, False, True, False)
